CREATE OR REPLACE VIEW mart.kdz_30_flights_full_data AS
	...
	to_char(fd.flight_scheduled_ts, 'MM'::text) AS flight_month
FROM
	mart.fact_departure fd 
		LEFT JOIN (SELECT weather_type_dk, cold, rain, snow, thunderstorm, drizzle, fog_mist FROM dds.weather_type) wt USING(weather_type_dk)
		LEFT JOIN (SELECT dwh_dk, src_iata_id as airport_origin_iata_id FROM dwh.id_airport) ia1 ON fd.airport_origin_dk = ia1.dwh_dk
		LEFT JOIN (SELECT dwh_dk, src_iata_id as airport_destination_iata_id FROM dwh.id_airport) ia2 ON fd.airport_destination_dk = ia2.dwh_dk
		LEFT JOIN kdz_30_dds.cancellation_code_id cci USING(cancellation_code)
